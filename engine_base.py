...
def evaluate(args, model, data_loader, device, record, epoch):
    ...
    for i, (inputs, target) in enumerate(tqdm(data_loader)):
        ...
        labels_support = Variable(torch.arange(0, args.n_way).long(), requires_grad=False).reshape(-1).to(device)
        labels_query = Variable(torch.arange(0, args.n_way).view(args.n_way, 1).expand(args.n_way, args.query).long(), requires_grad=False).reshape(-1).to(device)
